function hmri_corr_imperf_spoil_noGUI( param )

% modified by C.R. to bypass GUI, because I am lazy
...
FA      =   param.FA_deg;              % Flip angles [deg]
TR      =   param.TR_ms;               % [ms]
Phi0    =   param.Phi0_deg;            % [deg]
B1range =   param.B1range; % convert such that 100% = 1
Gdur    =   param.Gdur_ms;             % [ms]
Gamp    =   param.Gamp_mT_per_m;       % [[mT/m]
...
T1range     = param.T1range_ms;     %[ms]
T2range     = param.T2range_ms;     % [ms]
D           = param.D_um2_per_ms;   % [um^2/ms]
...
Results.Output.small_angle_approx = param.small_angle_approx;
Results.Output.RMSE_percent.T1app=round(RMSE_App,3);
Results.Output.RMSE_percent.T1corr=round(RMSE_Corr,3);

Results.ToCopy{1}    =['hmri_def.MPMacq_set.names{NN} = ''' param.prot_name ''';' ];
Results.ToCopy{end+1}=['hmri_def.MPMacq_set.tags{NN}  = ''' strrep(param.prot_name,' ','') ''';'];
Results.ToCopy{end+1}=['hmri_def.MPMacq_set.vals{NN}  = [' num2str([TR FA]) '];'];
Results.ToCopy{end+1}=['hmri_def.imperfectSpoilCorr.' strrep(param.prot_name,' ','') '.tag = ''' strrep(param.prot_name,' ','') ''';' ];
Results.ToCopy{end+1}=['hmri_def.imperfectSpoilCorr.' strrep(param.prot_name,' ','') '.P2_a = [' num2str(round(polyCoeffA,4)) '];'];
Results.ToCopy{end+1}=['hmri_def.imperfectSpoilCorr.' strrep(param.prot_name,' ','') '.P2_b = [' num2str(round(polyCoeffB,4)) '];'];
Results.ToCopy{end+1}=['hmri_def.imperfectSpoilCorr.' strrep(param.prot_name,' ','') '.small_angle_approx = ' mat2str(param.small_angle_approx) ';'];
Results.ToCopy{end+1}=['hmri_def.imperfectSpoilCorr.' strrep(param.prot_name,' ','') '.enabled = hmri_def.imperfectSpoilCorr.enabled;'];

results_filename = fullfile(param.outdir,[strrep(param.prot_name,' ',''),'.json']);

%% C.R. write coefficients to a .mat file for easier import
% keep the json as well to be able to check simulation parameters
save(fullfile(param.outdir,[strrep(param.prot_name,' ',''),'_ABcoeff.mat']),'polyCoeffA','polyCoeffB')

% Write json 
spm_jsonwrite(results_filename,Results,struct('indent','\t'));
