...
%% This ends up being a bit noisy, so we will set a cutoff threshold
...
% for j  = 1:length(MPRAGE_tr)
% 
%     % Find where formalin signal is nulled
%     idx = find(abs(y1(j,:)) == min(abs(y1(j,:))));
%     str = ['Formalin Null at ', num2str(inversiontimes(idx)),' ms'];
%     
%     figure; 
%     plot(inversiontimes,y1(j,:),'LineWidth',3);
%     hold on 
%     plot(inversiontimes,y2(j,:),'LineWidth',3);
%     plot(inversiontimes,y3(j,:),'LineWidth',3);
%         xline(inversiontimes(idx),'-',str)
%         yline( 0, '-')
%         ylim([-0.06 0.08])
%         xlabel('Inversion Time(ms)')
%         ylabel('Signal (a.u.)')
%         title(['MPRAGE Signal with TR = ', num2str(MPRAGE_tr(j))])
%         ax = gca;
%         ax.FontSize = 12; 
%     legend('Formalin', 'WM', 'GM','Location','best')
%     hold off
% end


%% If we want a null background, we can divide also by formalin signal
% % If we assume a noise level of 0.001, we can derive CNR and plot that. 
% Z2 =( (y2 - y3)./abs(y1))/ 0.001;
% 
% % First, look at signal of GM
% figure;
% surf(X,Y,Z2)
% xlabel('Inversion Time(ms)')
% ylabel('TR (ms)')
% zlabel('CNR/Formalin Signal')
