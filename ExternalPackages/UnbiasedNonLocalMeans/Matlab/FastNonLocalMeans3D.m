...
%
%   USAGE: out = FastNonLocalMeans( V, sigma [beta, rs, rc, ps, flag, block] )
%
...
for x=1:X
    for y=1:Y
        for z=1:Z
            ...
            gz0 = Gz(y,x,z);
            if( block==1 )
                % VECTOR IMPLEMENTATION (SEEMS TO BE FASTER FOR LARGE rs):
                ...
                % LOOP IMPLEMENTATION (SEEMS TO BE FASTER FOR SMALLER rs):
                pixel = 0.0;
                NORM  = 0.0;
                for s=mx:MX
                    for t=my:MY
