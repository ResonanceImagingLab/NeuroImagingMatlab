...
if cut<t
    ...
    view(90,180); %Medial, other hemisphere
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    %lighting flat; material dull; shading interp;
else
%     a(2)=axes('position',[1-0.055-h*3/4 0.62 h/r(2)*r(1)*3/4 w]);
%     trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),...
%         double(data),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
%     view(180,180);
%     daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
a(4)=axes('position',[0.3 0.40 w h]);
trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),...
    double(data),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(180,-90); %Dorsal
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
% 
if cut<t
    a(5)=axes('position',[1-0.055-h*3/4 0.29 h*3/4 w]);
    trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
        double(data(vr)),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
    view(-90,180); %Lateral other hemisphere
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    %lighting flat; material dull; shading interp;
% 
%     a(5)=axes('position',[0.055 0.02 w1 h1]);
%     trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),...
%         double(data),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
%     view(180,180); %Posterior view
%     daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    %lighting flat; material dull; shading interp;
% 
