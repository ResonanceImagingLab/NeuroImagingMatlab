function [ a, cb ] = SurfStatViewData_HCP(data_L, surf_L, data_R, surf_R, title, background);
...
r=max(surf_L.coord,[],2)-min(surf_L.coord,[],2);
w1=h/r(2)*r(1)*3/4;
h1=h/r(2)*r(1); % h/r(2)*r(3)

a(1)=axes('position',[0.055 0.62 h*3/4 w]); %Axes start and height
...
a(2)=axes('position',[0.25 0.45 w h]);
trisurf(surf_L.tri,surf_L.coord(1,:),surf_L.coord(2,:),surf_L.coord(3,:),...
    double(data_L),'EdgeColor','none', 'AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(0,90); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
a(4)=axes('position',[0.67 0.6 h*3/4 w]); %Axes start and height
...
a(5)=axes('position',[0.37 0.45 w h]);
trisurf(surf_R.tri,surf_R.coord(1,:),surf_R.coord(2,:),surf_R.coord(3,:),...
    double(data_R),'EdgeColor','none', 'AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(0,90); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
a(6)=axes('position',[0.67 0.29 h*3/4 w]);
trisurf(surf_R.tri,surf_R.coord(1,:),surf_R.coord(2,:),surf_R.coord(3,:),...
    double(data_R),'EdgeColor','none', 'AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(-90,0); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
set(cb,'position',[0.35 0.2 0.3 0.03]); % C.R. removed capital on position
set(cb,'XAxisLocation','bottom','FontSize',10);
h=get(cb,'Title');
