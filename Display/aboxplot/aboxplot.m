...
%% C.R. changes
% for stacked N multidatasets with M groups in each dataset,
% build cell array to have N rows and M columns
% X = nod_cat;

% Add optional input 'inputColour' if you want to specify the starting box
% colours. And example to use the standard first 3 matlab colours in most
% plotting functions would be to define: 
% inputColour = [0 0.4470 0.7410; 0.8500 0.3250 0.0980; 0.9290 0.6940 0.1250 ];
% Then call the function: 
% aboxplot(X,'labels',{'Label1', 'Label2', 'Label3'},'inputColour',inputColour);
% where X is a 1x3 cell
...
% Get the number or data matrices , CR modified
if iscell(X)
    %d = length(X);
    d = size(X,1); % CR new
else
...
if iscell(X)
    n = size(X{1},2);
    if n == 1 % C.R. correct for not grouped
        n = size(X,2);
    end
else
    n = size(X,3);
...
while i <= optargin
    switch lower(varargin{i})
        ...
        case 'widthe'
            widthe = varargin{i+1};
        case 'inputcolour'
            % C.R. add -> define cmap for each N
            inputColour = varargin{i+1};    
    end
    i = i + 2;
end
...
if exist( 'inputColour','var') % C.R. new:
    cmap = inputColour;
    if size(cmap,1) ~= n
        error('The number of inputColours in the colormap must equal n.');
    end
    for j=1:d
        for i=1:n
            colors{j,i} = cmap(i,:);
...
for j=1:d
    ...
    for i=1:n
        ...
        elseif iscell(X) && d == 1  % C.R new for single datasets
            
            % Calculate the mean and confidence intervals
            [q1 q2 q3 fu fl ou ol] = quartile(X{i});
            u = nanmean(X{i});
