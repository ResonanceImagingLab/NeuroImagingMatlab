...
figure;
    ...
    xlim([min(input1)-min(input1)*0.02 max(input1+0.02*max(input1))])
...
subplot(1,2,2)
    ...
    caption1 = strcat('-1.96SD =',num2str(BA_mean - BA_lineshift, '%.2e'));
    caption2 = strcat('+1.96SD =',num2str(BA_mean + BA_lineshift, '%.2e') ); % , '(+1.96SD)'
    caption3 = strcat('Mean =',num2str(BA_mean, '%.2e'));
    
    xlocText = min(BA_xval)*0.98 + 0.01*min(BA_xval)*0.98;
    text( xlocText, BA_mean - BA_lineshift+(0.1*BA_lineshift), caption1, 'FontSize', 16,'FontWeight','bold');
    text( xlocText, BA_mean + BA_lineshift+(0.1*BA_lineshift), caption2, 'FontSize', 16,'FontWeight','bold');
    text( xlocText, BA_mean + (0.1*BA_lineshift), caption3, 'FontSize', 16,'FontWeight','bold');
